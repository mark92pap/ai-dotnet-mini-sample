...
public record UpdateStoreItemCommand(int Id, UpdateStoreItemRequest Request) : IRequest<Result<StoreItemResponse>>;

public class UpdateStoreItemCommandHandler : IRequestHandler<UpdateStoreItemCommand, Result<StoreItemResponse>>
{
    private readonly IStoreItemRepository _repository;

    public UpdateStoreItemCommandHandler(IStoreItemRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<StoreItemResponse>> Handle(UpdateStoreItemCommand request, CancellationToken cancellationToken)
    {
        // Validate price
        ...
        var item = StoreItemMapper.ToDomain(request.Id, request.Request);

        var result = await _repository.UpdateAsync(item, cancellationToken);
